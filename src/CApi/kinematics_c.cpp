// Implementation of the Kinematics C ABI (see kinematics_c.h).
//
// This is the only non-header translation unit in the engine: it bridges the
// flat C surface to the C++ classes. Built into the shared/static `kinematics_c`
...
// ── World lifecycle ──────────────────────────────────────────────
KnWorld* kn_world_create(void) { return new KnWorld(); }
void kn_world_destroy(KnWorld* world) { delete world; }

void kn_world_set_limits(KnWorld* world, float minX, float minY, float maxX, float maxY) {
    if (world) world->ctrl.SetWorldLimits(Vector2(minX, minY), Vector2(maxX, maxY));
}

void kn_world_set_friction(KnWorld* world, float friction) {
    if (world) world->ctrl.Friction = friction;
}
...
int kn_world_add_pressure_body(KnWorld* world, const float* xy, int count, float mass,
                               float gasPressure, float edgeK, float edgeDamp,
                               float shapeK, float shapeDamp, float posX, float posY,
                               float gravX, float gravY, int isStatic) {
    if (!world || !xy || count < 3) return -1;
...
void kn_world_set_chain_gravity(KnWorld* world, float gx, float gy) {
    if (world) world->chainGravity = Vector2(gx, gy);
}
...
int kn_body_get_points(KnWorld* world, int body, float* outXY, int maxPoints) {
    if (!validBody(world, body) || !outXY || maxPoints < 0) return 0;
...
void kn_body_get_position(KnWorld* world, int body, float* outX, float* outY) {
    if (!validBody(world, body)) return;
    if (outX) *outX = world->ctrl.BodyList[body]->Position.X;
    if (outY) *outY = world->ctrl.BodyList[body]->Position.Y;
}
...
int kn_chain_point_count(KnWorld* world, int chain) {
    if (!validChain(world, chain)) return 0;
...
int kn_chain_get_points(KnWorld* world, int chain, float* outXY, int maxPoints) {
    if (!validChain(world, chain) || !outXY || maxPoints < 0) return 0;
